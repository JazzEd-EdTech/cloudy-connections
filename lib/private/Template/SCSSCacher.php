...
class SCSSCacher {
	...
	public function process(string $root, string $file, string $app): bool {
		...
		if (!$this->lockingCache->add($lockKey, 'locked!', 120)) {
			...
			while ($retry < 10) {
				if (!$this->variablesChanged() && $this->isCached($fileNameCSS, $app)) {
					...
					$this->logger->debug('SCSSCacher: ' .$lockKey.' is now available after '.$retry.'s. Moving on...', ['app' => 'scss_debug']);
					return $this->injectCssVariablesIfAny();
				}
				$this->logger->debug('SCSSCacher: scss cache file locked for '.$lockKey, ['app' => 'scss_debug']);
				sleep(1);
				$retry++;
			}
			$this->logger->debug('SCSSCacher: Giving up scss caching for '.$lockKey, ['app' => 'scss_debug']);
			return false;
		}

		$this->logger->debug('SCSSCacher: Lock acquired for '.$lockKey, ['app' => 'scss_debug']);
		try {
		...
		// Cleaning lock
		$this->lockingCache->remove($lockKey);
		$this->logger->debug('SCSSCacher: Lock removed for '.$lockKey, ['app' => 'scss_debug']);
	...
	private function isCached(string $fileNameCSS, string $app) {
		...
		$this->logger->error("SCSSCacher::isCached isCachedCache is expired or unset", ['app' => 'scss_debug']);
